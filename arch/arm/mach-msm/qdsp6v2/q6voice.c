...
static int voice_create_mvm_cvs_session(struct voice_data *v)
{
	...
	if (!cvs_handle) {
		if (v->voc_path == VOC_PATH_PASSIVE) {
			q6voice_info("%s:creating CVS passive session\n", __func__);
		...
		q6voice_info("send stream create session pkt size = %d\n",
		...
		if (!ret) {
			...
			q6voice_info("%s:creating CVS full session\n", __func__);
			...
			/* Attach MVM to CVS. */
			q6voice_info("%s: Attach MVM to stream\n", __func__);
...
static int voice_destroy_mvm_cvs_session(struct voice_data *v)
{
	...
	if (v->voc_path == VOC_PATH_FULL) {
		q6voice_info("%s: MVM detach stream\n", __func__);
		...
		/* Destroy CVS. */
		q6voice_info("%s: CVS destroy session\n", __func__);
		...
		/* Destroy MVM. */
		q6voice_info("%s: MVM destroy session\n", __func__);
...
static int voice_send_tty_mode_to_modem(struct voice_data *v)
{
	...
	if (tty_mode) {
		...
		q6voice_info("tty mode =%d\n", mvm_tty_mode_cmd.tty_mode.mode);

		v->mvm_state = CMD_STATUS_FAIL;
		q6voice_info("%s: MVM set tty\n", __func__);
...
static int voice_config_cvs_vocoder(struct voice_data *v)
{
	...
	q6voice_info("%s: Setting media type\n", __func__);
	...
	case VSS_MEDIA_ID_EVRC_MODEM: {
		...
		q6voice_info("%s: Setting EVRC min-max rate\n", __func__);
	...
	case VSS_MEDIA_ID_AMR_NB_MODEM: {
		...
		q6voice_info("%s: Setting AMR rate\n", __func__);
	...
	case VSS_MEDIA_ID_AMR_WB_MODEM: {
		...
		q6voice_info("%s: Setting AMR WB rate\n", __func__);
...
static int voice_send_start_voice_cmd(struct voice_data *v)
{
	...
	q6voice_info("send mvm_start_voice_cmd pkt size = %d\n",
...
static int voice_send_stop_voice_cmd(struct voice_data *v)
{
	...
	q6voice_info("send mvm_stop_voice_cmd pkt size = %d\n",
...
static int voice_setup_modem_voice(struct voice_data *v)
{
	...
	q6voice_info(" send create cvp session, pkt size = %d\n",
	...
	q6voice_info("topology=%d net_id=%d, dir=%d tx_port_id=%d, rx_port_id=%d\n",
			cvp_session_cmd.cvp_session.tx_topology_id,
...
static int voice_destroy_modem_voice(struct voice_data *v)
{
	...
	q6voice_info("mvm_d_vocproc_cmd  pkt size = %d\n",
	...
	q6voice_info("cvp_destroy_session_cmd pkt size = %d\n",
			...
			void *private_data)
{
	...
	q6voice_info("auddev_cb_function, evt_id=%d,\n", evt_id);
...
uint32_t voice_get_config_vocoder_rate()
{
       return voice.mvs_info.rate;
}
		...
		if (data->payload_size) {
			...
			ptr[0] ==
				...
				if (!ptr[1]) {
					...
					q6voice_info("got NACK for sending \
							MVM create session \n");
				v->mvm_state = CMD_STATUS_SUCCESS;
				wake_up(&v->mvm_wait);
		...
		if (data->payload_size) {
			...
			if (ptr[0] ==
				...
				if (!ptr[1]) {
					...
					q6voice_info("got NACK from CVP create \
						session response\n");
				v->cvp_state = CMD_STATUS_SUCCESS;
				wake_up(&v->cvp_wait);
			} else if (ptr[0] ==
