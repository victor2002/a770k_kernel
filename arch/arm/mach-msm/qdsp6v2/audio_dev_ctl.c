					...
					int rate, int mode)
{
	...
	if (set) {
#ifdef CONFIG_RECINCALL
		rc = adm_open(copp_id, PLAYBACK, rate, mode, DEFAULT_COPP_TOPOLOGY);
#else
		rc = adm_open(copp_id, popp_id, PLAYBACK, rate, mode, DEFAULT_COPP_TOPOLOGY);
#endif
		if (rc < 0) {
...
#if 0//def CONFIG_SKYSND_CTRL
	case AUDIO_SKY_LOOPBACK_EN: {
		u32 loop;
              pr_err("AUDIO_SKY_LOOPBACK_EN\n");
		if (get_user(loop, (u32 __user *) arg)) {
			rc = -EFAULT;
			break;
		}
	       sky_debugfs_adie_loopback(loop);  
              //snd_extamp_api_SetDevice(loop, SND_DEVICE_SPEAKER_RX);
		break;
	}
#endif
	default:
