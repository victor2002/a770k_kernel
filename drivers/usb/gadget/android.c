...
#define MAX_STR_LEN		255 //p13120 16 -> 255
...
static ssize_t usb_serial_write_show(struct device *dev,
																struct device_attribute *attr, const char *buf)
{
	return 0;
}

static ssize_t usb_serial_write_store(struct device *dev,
...
static DEVICE_ATTR(usb_serial_write,  
		S_IWUSR | S_IRUSR | S_IXUSR |
		S_IRGRP | S_IWGRP | S_IXGRP , 
		usb_serial_write_show , usb_serial_write_store);
...
static int __init android_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		...
		result = sysfs_create_group(&pdev->dev.kobj, &android_pantech_usb_control_attr_grp);
		if (result < 0) {
