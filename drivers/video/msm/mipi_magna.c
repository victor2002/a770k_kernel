...
static int mipi_magna_lcd_on(struct platform_device *pdev)
{
	...
	if (magna_state.disp_initialized == false) {
		...
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_init_cmds,
	...
	if (magna_state.disp_veil_enable == true) {
		...
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_veil_init0_cmds,
				ARRAY_SIZE(magna_display_veil_init0_cmds));
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_veil_lut_cmds[veil_state.lut],
				ARRAY_SIZE(magna_display_veil_lut_cmds[veil_state.lut]));
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_veil_init1_cmds,
				ARRAY_SIZE(magna_display_veil_init1_cmds));
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_veil_tex_cmds[veil_state.tex],
				ARRAY_SIZE(magna_display_veil_tex_cmds[veil_state.tex]));
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_veil_colormap_cmds[veil_state.map],
				ARRAY_SIZE(magna_display_veil_colormap_cmds[veil_state.map]));
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_veil_init2_cmds,
				ARRAY_SIZE(magna_display_veil_init2_cmds));
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_veil_on_cmds,
	...
	if (magna_state.disp_veil_enable == false && magna_state.gamma != 0) {
		...
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_otp_non_load_cmds,
				ARRAY_SIZE(magna_otp_non_load_cmds));
    }
#endif
	if (magna_state.disp_on == false) {
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_sleep_out_cmds,
		...
		if (magna_state.disp_veil_enable == false && magna_state.gamma != 0) {
			...
			mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_otp_non_init_cmds,
					ARRAY_SIZE(magna_otp_non_init_cmds));

			mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_gamma_cmds[magna_state.gamma],
					ARRAY_SIZE(magna_display_gamma_cmds[magna_state.gamma]));
		}
#endif
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_on_cmds,
...
static int mipi_magna_lcd_off(struct platform_device *pdev)
{
	...
	if (magna_state.disp_on == true) {
		...
		mutex_lock(&mfd->dma->ov_mutex);
		mipi_dsi_cmds_tx(mfd, &magna_tx_buf, magna_display_off_cmds,
...
//			local_save_flags(flags); //2011.05.30 leecy  πÿ¿∏∑Œ ¿Ãµø..
//			local_irq_disable();
			if (prev_bl_level == 0) {
				//count++;
