...
#if (0)//FEATURE_PANTECH_MMP_QCOM_CR TestSBA_M8660AAABQNLYA109010_Pantech_EF33S-EF34K_05202011_Case00518255.zip
	DDL_MSG_ERROR("[PANTECH] decoder->codec.codec(0x%x), ddl_vidc_decode_set_buffers()", decoder->codec.codec);
	if (decoder->codec.codec == VCD_CODEC_XVID || decoder->codec.codec == VCD_CODEC_MPEG4)
		vidc_sm_set_mpeg4_profile_override(
			&ddl->shared_mem[ddl->command_channel],
			 VIDC_SM_PROFILE_INFO_ASP);
#else
	DDL_MSG_ERROR("[PANTECH] Rollback... decoder->codec.codec(0x%x), ddl_vidc_decode_set_buffers()", decoder->codec.codec);
#endif
