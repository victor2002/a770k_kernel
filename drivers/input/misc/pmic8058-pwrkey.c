...
static struct pm8058_chip	*pwrkey_pm_chip;

int pm8058_get_pwrkey_status(void)
{
        return pm8058_irq_get_rt_status(pwrkey_pm_chip, PM8058_PWRKEY_PRESS_IRQ(PM8058_IRQ_BASE));
}
...
//	printk("Current Status: %d\n", pm8058_get_pwrkey_status());
...
#else
	if(pm8058_get_pwrkey_status() == false)	//Release Event
	{
		printk("pmic8058_pwrkey_timer We have to change to Key Release Status\n");
	...
	{	
		hrtimer_start(&pwrkey->timer,
				ktime_set(POWER_CHECK_TIMER / 1000,
				(POWER_CHECK_TIMER % 1000) * 1000000),
				HRTIMER_MODE_REL);
	}
#endif
