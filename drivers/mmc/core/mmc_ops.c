...
#ifdef CONFIG_MMC_DISCARD_MOVINAND //PS1 TEAM SHS : SAMSUNG PATCH
#ifdef CONFIG_MMC_DISCARD_VENDOR_OPTSIZE
...
int mmc_send_trimsize(struct mmc_card *card, u32 *trimsize)
{
	...
	*trimsize = 0;
	...
	ret = mmc_send_trimsize_cmd(card,  0x00DECCEE);
	if (ret)
		return ret;
	...
	ret = mmc_send_trimsize_cmd(card, 0x0000CCEE);
	if (ret)
		return ret;
	...
	*trimsize = 0;
    ...
    {
		*trimsize = card->ext_csd.hc_wp_grp_size * 1024 / 32;  /* in sectors */
    }
#endif

	return 0;
}
#endif /* CONFIG_MMC_DISCARD_MOVINAND */ //PS1 TEAM SHS : SAMSUNG PATCH
