...
extern int max17040_get_charge_state();
#endif
#ifdef CONFIG_SKY_CHARGING
...
{
#ifdef CONFIG_SKY_CHARGING
	dev_info(msm_chg.dev, "%s resuming charging %d", __func__,
...
//#ifdef CONFIG_SKY_CHARGING
#if 0
        pmic8058_tz_get_temp_charging(&temp);

	pr_info("[SKY CHG]msm-charger %s batt_status= %d, temperature= %d\n",
				__func__, msm_chg.batt_status, temp);
#endif

	pr_info("msm-charger %s batt_status= %d, charger_type =%d\n",
				__func__, msm_chg.batt_status, msm_chg.charger_type);
...
//#ifdef CONFIG_SKY_CHARGING
#if 0
                if(msm_chg.charger_type == CHG_TYPE_AC)
                {
                        if(is_lcd_on())
                                pm8058_chg_set_current_temperature(700);
                        else
                                pm8058_chg_set_current_temperature(900);

                	pr_info("msm-charger %s lcd_status= %d\n",
                				__func__, is_lcd_on());
                }
#endif
	}
