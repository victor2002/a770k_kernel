...
static void bluesleep_sleep_work(struct work_struct *work)
{
	if (bluesleep_can_sleep()) {
		...
		if (test_bit(BT_ASLEEP, &flags)) {
			//printk(KERN_INFO"%s : already asleep \n",__func__);
			return;
		}
...
{
	//printk(KERN_INFO"%s : hostwake line change \n",__func__);
...
static void bluesleep_tx_timer_expire(unsigned long data)
{
	...
	//printk(KERN_INFO"%s : Tx timer expired \n",__func__);

	/* were we silent during the last timeout? */
	if (!test_bit(BT_TXDATA, &flags)) {
		//printk(KERN_INFO"%s : Tx has been idle \n",__func__);
#ifndef BRCM_H4_LPM_SUPPORT
		gpio_set_value(bsi->ext_wake, 1);
#endif
		bluesleep_tx_idle();
	} else {
		//printk(KERN_INFO"%s : Tx data during last period \n",__func__);
		mod_timer(&tx_timer, jiffies + (TX_TIMER_INTERVAL*HZ));
...
static int bluesleep_start(void)
{
	...
	//				IRQF_DISABLED | IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING,"bt_hostwake", NULL);
	...
	if (retval < 0) {
		//printk(KERN_INFO"%s : Couldn't enable BT_HOST_WAKE as wakeup interrupt \n",__func__);
...
#ifdef BRCM_H4_LPM_SUPPORT
		if (test_bit(BT_ASLEEP, &flags)) {
			//printk(KERN_INFO"%s : Wake-up UART first if asleep \n",__func__);
		    bluesleep_outgoing_data();
		}
#endif
		gpio_set_value(bsi->ext_wake, 0);
...
static int __init bluesleep_probe(struct platform_device *pdev)
{
	...
	request_irq(bsi->host_wake_irq, bluesleep_hostwake_isr,
					IRQF_DISABLED | IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING,"bt_hostwake", NULL);
...
static int __init bluesleep_init(void)
{
	...
	//printk(KERN_INFO"%s : MSM Sleep Mode Driver Ver %s \n",__func__, VERSION);
	...
	if (bluetooth_dir == NULL) {
		//printk(KERN_INFO"%s : Unable to create /proc/bluetooth directory \n",__func__);
		return -ENOMEM;
	}

	sleep_dir = proc_mkdir("sleep", bluetooth_dir);
	if (sleep_dir == NULL) {
		//printk(KERN_INFO"%s : Unable to create /proc/%s directory \n",__func__, PROC_DIR);
		return -ENOMEM;
	}
	...
	if (ent == NULL) {
		//printk(KERN_INFO"%s : Unable to create /proc/%s/btwake entry \n",__func__);
		retval = -ENOMEM;
		goto fail;
	}
	...
	if (create_proc_read_entry("hostwake", 0, sleep_dir,
		...
		//printk(KERN_INFO"%s : Unable to create /proc/%s/hostwake entry \n",__func__, PROC_DIR);
		retval = -ENOMEM;
		goto fail;
	}
	...
	if (ent == NULL) {
		//printk(KERN_INFO"%s : Unable to create /proc/%s/proto entry \n",__func__, PROC_DIR);
		retval = -ENOMEM;
		goto fail;
	}
	...
	if (create_proc_read_entry("asleep", 0,
		...
		//printk(KERN_INFO"%s : Unable to create /proc/%s/asleep entry \n",__func__, PROC_DIR);
		retval = -ENOMEM;
		goto fail;
	}
