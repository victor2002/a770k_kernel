...
#define S5K6AAFX13_MAX_PREVIEW_FPS 14
	...
	if((preview_fps > S5K6AAFX13_MAX_PREVIEW_FPS) && (preview_fps < C_SKYCAM_MAX_PREVIEW_FPS))
...
static long s5k6aafx13_set_sensor_mode(int mode)
{
	...
	case SENSOR_PREVIEW_MODE:
		...
		if(config_csi == 0)
		{
			...
			CAM_INFO("s5k6aafx13_video_config, rc = %d \n", rc);
			if (rc < 0)
	              {
	...
	case SENSOR_SNAPSHOT_MODE:
		if(!b_snapshot_flag) {
			...
			CAM_INFO("s5k6aafx13_snapshot_config, rc = %d \n", rc);
			if (rc < 0)
	              {
...
#if defined(F_SKYCAM_TARGET_EF33S) || defined(F_SKYCAM_TARGET_EF34K)
#if BOARD_VER_GE(TP15)//change to TP20 at SMT TP20 by KHY				
	s->s_mount_angle = 270;
#else
	s->s_mount_angle = 0;//180;//90;
#endif				
#elif defined(F_SKYCAM_TARGET_EF35L)
#if BOARD_VER_G(WS20)
	s->s_mount_angle = 270;
#else
	s->s_mount_angle = 0;//180;//90;
