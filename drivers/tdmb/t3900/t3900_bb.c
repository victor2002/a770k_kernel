...
static boolean t3700_function_register(tdmb_bb_function_table_type *ftable_ptr)
{
  ...
  ftable_ptr->tdmb_bb_set_antenna_path  = t3700_set_antenna_path;
...
#ifdef FEATURE_QTDMB_I2C_CMD
  ret = tdmb_i2c_write(chipid, reg, sizeof(uint16), data, sizeof(uint16));
#endif /* FEATURE_QTDMB_I2C_CMD */
...
{
#ifdef FEATURE_QTDMB_I2C_CMD
  tdmb_i2c_read(chipid, reg, sizeof(uint16), data, sizeof(uint16));
#endif /* FEATURE_QTDMB_I2C_CMD */
...
{
#ifdef FEATURE_QTDMB_I2C_CMD
  tdmb_i2c_read_len(chipid, uiAddr, ucRecvBuf, ucCount, sizeof(uint16));
#endif /* FEATURE_QTDMB_I2C_CMD */
...
#ifdef FEATURE_TDMB_PMIC_POWER
/*====================================================================
FUNCTION       t3700_pmic_power_onoff
DESCRIPTION 
DEPENDENCIES
RETURN VALUE
SIDE EFFECTS
======================================================================*/
static boolean t3700_pmic_power_onoff(int on)
{
  unsigned mpp ;
  ...
  if (on)
  {
    ...
    TDMB_MSG_INC_BB("[%s] vreg set 1.2V\n", __func__);
  }
...
SIDE EFFECTS
======================================================================*/
static boolean t3700_pmic_ant_switch_power(int on)
{
  static struct regulator *vreg; //If verg value was changed, can not disable regulator
  int ret = 0;
  
  TDMB_MSG_INC_BB("[%s] on[%d]!!!\n", __func__, on);

  if(on)
  {
    vreg = regulator_get(NULL, "8058_l11");

    ret = regulator_set_voltage(vreg, 2000000, 2000000);
    
    if(!ret) ret = regulator_enable(vreg);
    if(ret)
    {
      TDMB_MSG_INC_BB("TDMB regulator_enable Fail [%d] !!!\n",ret);
    }
  }
  else
  {
    ...
    if(ret)
    {
      TDMB_MSG_INC_BB("TDMB regulator_disable Fail [%d] !!!\n",ret);
    }
  }
...
#ifdef FEATURE_TDMB_SET_ANT_PATH
  t3700_pmic_ant_switch_power(TRUE);
#endif
...
#ifdef FEATURE_TDMB_SET_ANT_PATH
  t3700_pmic_ant_switch_power(FALSE);
...
/*====================================================================
FUNCTION       t3700_set_antenna
DESCRIPTION 
DEPENDENCIES
RETURN VALUE
SIDE EFFECTS
======================================================================*/
void t3700_set_antenna_path(int ant_type)
{
#ifdef FEATURE_TDMB_SET_ANT_PATH
...
#else
  //EF33S/34K Since WS20 Use only retractable Antenna
...
int t3700_read_msc(uint8 *msc_buffer)
{
  ...
  if( m_ucCommandMode == INC_EBI_CTRL)
  {
    ...
    {
      TDMB_MSG_INC_BB("[%s] occur interrupt less than INC_INTERRUPT_SIZE %d Status %d\n", __func__, uiLoop, wOperState);
      return 0;
    }
  ...
  {
    uiSize = INC_GET_CHANNEL_COUNT(FIC_STREAM_DATA);
    //TDMB_MSG_INC_BB("[%s] GET FIC SIZE %d\n", __func__, uiSize);
    if(uiSize){
      ts_data.fic_size = uiSize;
      ts_data.type = FIC_DATA;

      //TDMB_MSG_INC_BB("[%s] end. ts_data.type[%d]\n", __func__, ts_data.type);

      INC_GET_CHANNEL_DATA(FIC_STREAM_DATA, msc_buffer, uiSize);

      if(uiSize >= BB_MAX_FIC_SIZE){
#ifdef USE_FEATURE_PHONE_FIC_PROC_IN_MULTI
        INC_QFIFO_ADD(g_pStFifo, msc_buffer, uiSize);
        fic_data_ready = TRUE;
#else
        memcpy(ts_data.fic_buf, msc_buffer, uiSize);
#endif /* USE_FEATURE_PHONE_FIC_PROC_IN_MULTI */
      }
      else {
    ...
    uiSize = INC_GET_CHANNEL_COUNT(CHANNEL1_STREAM_DATA);
    //TDMB_MSG_INC_BB("[%s] GET CH1 SIZE %d\n", __func__, uiSize);
    if(uiSize){
      ts_data.msc_size = uiSize;
      ...
      INC_GET_CHANNEL_DATA(CHANNEL1_STREAM_DATA, msc_buffer, uiSize);

#ifdef FEATURE_TS_PKT_MSG
      TDMB_MSG_INC_PKT("[%s] [%cch.%ccmd] end. msc_buf[0](0x%.2X 0x%.2X 0x%.2X 0x%.2X)\n", __func__,
...
int t3700_Ant_Level(uint32 pcber)
{
  ...
  else if(pcber >= ant_tbl[5] && pcber < ant_tbl[4]) level = 5;
  else if(pcber < ant_tbl[5]) level = 6;
   
  if((level > 1) && (level == save_level + 1))
  {
